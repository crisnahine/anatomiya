 ...
 *
 * A key that is not written out is the same fact from the other side. `key =>
 * false`, `CONST => 1` and `:"#{name}" => 1` each hide which option was set, so
 * one of them can be the very option the row asks for. Skipping the entry and
 * returning the rest hands the row a list it can read, and the row then convicts
 * a site nobody can grade. Declining under-counts the population, which
 * `N of N sites across X of Y files` already reports.
 */
...
function foreignKeys(cls) {
  ...
  walkRuby(cls.body, (m, mctx) => {
    ...
    // Whether the two facts that match it to a reference can be read: the table
    // it is added to, and the column it covers, which is `column:` where it
    // names one and the table it points at otherwise. A statement missing
    // either is kept rather than dropped, because it may be the key covering a
    // reference below and nothing can prove it is not.
    const matchable = from !== null && opts !== null && (opts.has("column") ? column !== null : to !== null);
    out.push({ from, to, column, matchable });
  });
  return out;
}

// Whether a key covers this reference's column: the one it names outright, or
// the table the reference's own name pluralises to.
const covers = (k, name) =>
...
const declaredApart = (keys, table, name) =>
  table !== null && name !== null && keys.some((k) => k.matchable && k.from === table && covers(k, name));

// Whether one of them may be this reference's key without saying so, which is
// what stops the row charging a reference against evidence it could not read.
// A statement it could not read is on this table wherever its own table is
// unreadable, and where the reference's table is the unreadable one, any
// statement covering the column it names may be the key.
const mayCover = (keys, table, name) =>
  name === null
    ? keys.length > 0
    : keys.some((k) =>
        k.matchable ? table === null && covers(k, name) : table === null || k.from === null || k.from === table
      );
...
export const RAILS_DIMENSIONS = [
...

  {
    ...
    applicabilityPredicate: {
      ...
      notCounted:
        "a column whose options this tool cannot read: a ** splat, or a key that is not a literal name",
      blind: "a repository-local base class hides the migration from the superclass test",
    },
    langs: ["ruby"],
    run(ast, add) {
...

  {
    ...
    precision: "partial",
    applicabilityPredicate: {
      sites: "a migration class adding a reference column, through add_reference or add_belongs_to or the matching t. call inside a create_table or change_table block, and only where its options list can be read, and where its key is settled: declared in those options, matched to a statement of its own, or ruled out because no statement in the class could be it. A polymorphic reference is not one: ActiveRecord refuses a foreign key on it",
      notCounted:
        "a reference marked polymorphic: true, and one whose options this tool could not read, or whose key it could not settle",
      blind: "a repository-local base class hides the migration from the superclass test",
    },
    langs: ["ruby"],
    run(ast, add) {
      eachMigration(ast, (cls) => {
        ...
        walkRuby(cls.body, (m, mctx) => {
          ...
          const apart = declaredApart(keys, table, name);
          if (!inline && !apart && mayCover(keys, table, name)) return;
