...
function foreignKeys(cls) {
  ...
  walkRuby(cls.body, (m, mctx) => {
    ...
    // The table it is added to, the table it points at and the column it names
    // can each be written as something with no literal to read, and a statement
    // missing any of them may still be the key covering a reference below it.
    // Kept as evidence that cannot be matched rather than dropped, so the row
    // declines that reference instead of charging it with a missing key.
    const unreadable = from === null || to === null || opts === null;
    out.push({ from, to, column: unreadable ? null : lit(opts.get("column")), unreadable });
  });
  return out;
}

// Whether one of them covers this reference: the same table, and either the
// column named outright or the table the reference's own name pluralises to. A
// statement this tool could not read answers for nothing, since the column it
// covers may be another one; it blocks the reference below instead.
...
// Whether one of them could be this reference's key without saying so: a
// statement whose own table could not be read is on every table.
const unmatchable = (keys, table) => keys.some((k) => k.unreadable && (k.from === null || k.from === table));
...
export const RAILS_DIMENSIONS = [
...

  {
    ...
    precision: "partial",
    applicabilityPredicate: {
      sites: "a migration class adding a reference column, through add_reference or add_belongs_to or the matching t. call inside a create_table or change_table block, and only where its options list can be read, and where every foreign key the migration declares apart names the table it is added to. A polymorphic reference is not one: ActiveRecord refuses a foreign key on it",
      notCounted:
        "a reference marked polymorphic: true, and one whose options, or the foreign key covering it, could not be read",
      blind: "a repository-local base class hides the migration from the superclass test",
    },
    langs: ["ruby"],
    run(ast, add) {
      eachMigration(ast, (cls) => {
        ...
        walkRuby(cls.body, (m, mctx) => {
          ...
          // Evidence on the site itself is readable whatever else the class
          // holds, so only a reference that would be charged is declined.
          if (!inline && !apart && unmatchable(keys, table)) return;
