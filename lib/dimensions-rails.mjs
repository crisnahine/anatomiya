import { walkRuby, constName, ownDef } from "./ruby.mjs";
...
export const RAILS_DIMENSIONS = [
  {
    ...
    applicabilityPredicate: {
      sites: "a migration class defining at least one of change, up or down. A helper-only class has made no choice about reversibility, so counting it would pad the denominator with classes the claim is not about",
...

  {
    ...
    precision: "partial",
    applicabilityPredicate: {
      sites: "a migration class adding a column, through add_column or inside a create_table or change_table block. change_column and change_column_null alter a column that already exists rather than declare a new one, and t.timestamps and t.references are not nullability the writer chose",
      blind: "a repository-local base class hides the migration from the superclass test",
    },
    langs: ["ruby"],
    run(ast, add) {
...

  {
    ...
    precision: "partial",
    applicabilityPredicate: {
      sites: "a migration class adding a reference column, through add_reference or add_belongs_to or the matching t. call inside a create_table or change_table block",
      blind: "a repository-local base class hides the migration from the superclass test",
    },
    langs: ["ruby"],
    run(ast, add) {
