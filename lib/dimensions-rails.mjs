...
function foreignKeys(cls) {
  ...
  walkRuby(cls.body, (m, mctx) => {
    ...
    out.push({ from, to, column, unknownColumn });
  });
  return out;
}
...
const declaredApart = (keys, table, name) =>
  table !== null && name !== null && keys.some((k) => !k.unknownColumn && k.from === table && covers(k, name));
 ...
 */
const unsettled = (keys, table, name) =>
  keys.some(
    (k) =>
    ...
    run(ast, add) {
      eachMigration(ast, (cls) => {
        ...
        walkRuby(cls.body, (m, mctx) => {
          ...
          if (!inline && !apart && unsettled(keys, table, name)) return;
