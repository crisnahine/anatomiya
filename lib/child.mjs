...
export function guardedChild({
  ...
  if (typeof stderrBytes !== "number") throw new TypeError("guardedChild needs a stderrBytes cap");
  // Otherwise a misspelled kind reaches `spawn` with an undefined command, and
  // the error names that rather than the field that was wrong.
  if (kind !== "fork" && kind !== "spawn") throw new TypeError(`guardedChild has no kind ${JSON.stringify(kind)}`);
  ...
  return { child, stderr: () => whole(text), touch, kill, killedBy: () => killedBy, settle };
}

/**
 * The text without the half character an exact cap can land on: an astral
 * character is two UTF-16 code units, and a cut on the count alone leaves a
 * lone surrogate no earlier path could produce (B5).
 */
function whole(text) {
