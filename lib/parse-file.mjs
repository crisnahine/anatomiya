...
import { ENGINES, grammarFor, mayHoldFlow } from "./langs.mjs";
...
export async function parseFile(source, rel, lang, { withProgram = false, stripper } = {}) {
  ...
  // A tree that came back from the retry has its annotations blanked out, and
  // the rows that depend on them can only answer wrongly: two ask about the
  // annotation itself, and the third counts import statements the stripper
  // deletes outright. They are dropped for that file rather than counting a
  // confident zero; every dimension that reads code is unaffected.
