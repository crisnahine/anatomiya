 ...
 */
import { assertPrecision, assertApplicability } from "./dimensions.mjs";
...
const rails = (key, claim, from, to, companionSuffix = "_spec.rb", ext = ".rb") => ({
  ...
  applicability: {
    ...
    // both find eligible files in every Rails tree and one of them can only ever
    // read zero, which is a false statement rather than a measurement.
    sites: `a ${ext} file anywhere under ${from}, once the repository is seen using the ${companionSuffix} suffix at all`,
    blind: null,
  },
  langs: ["ruby"],
  from,
...
export const PAIRINGS = assertApplicability(
  assertPrecision([
  rails("rake_task_spec", "a rake task ships with a spec", "lib/tasks", "spec/lib/tasks", "_spec.rb", ".rake"),
...
export function companionRoot(corpus, { from, to, ext, companionSuffix }) {
  ...
  return tied.includes(to) ? to : null;
}
 ...
 */
export function pairingViolations(changed, corpus, pairing, root = null) {
  // Resolved here rather than in a default parameter: learning the root walks
  // the whole corpus, and a caller reading the signature would not see it.
  const at = root ?? companionRoot(corpus, pairing) ?? pairing.to;
  const out = [];
  for (const path of changed) {
    const companion = companionOf(path, pairing, at);
