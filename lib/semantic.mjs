 ...
 * the engine table owns every install sentence, and this module is one a forked
 * child loads, so it may not import the half that spawns processes.
 */
...
export function runSemantic(
  ...
  return new Promise((resolve) => {
    ...
    // `execArgv: []` for the same reason the parse pool pins it: a flag the
    // parent took that a file-loading child cannot leaves the worker stillborn
    // on every run. stderr is piped, so it is read rather than left to fill a
    // 64KB buffer and stall the child into a false idle timeout.
    const child = fork(workerPath, [], {
      // Outside the repository, per the F5 battery. Named rather than inlined
      // so the spawn-failure path has a way to be reached from a test: a cwd
      // that is not there is what makes fork emit 'error' rather than exit.
      cwd,
      execArgv: [],
