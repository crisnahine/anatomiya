...
export const EXTRA_DIMENSIONS = [
  {
    ...
    applicabilityPredicate: {
      sites: "a file declaring a function at module level, either as a declaration or as a binding initialised with one",
      blind: null,
    },
...

  {
    ...
    applicabilityPredicate: {
      ...
      blind: "a name used in both positions, or re-exported, is not decidable from this file alone",
    },
    // The whole question is the annotation, so a tree whose annotations were
    // blanked can only answer it wrongly. `parse-worker.mjs` drops this row for
    // such a file rather than counting a confident zero.
...

  {
    ...
    precision: "precise",
    applicabilityPredicate: {
      sites: "a file holding a || or ?? expression whose right side is a literal, an array or an object",
      blind: null,
    },
    langs: ["js", "jsx"],
    run(program, add) {
...

  {
    ...
    precision: "partial",
    applicabilityPredicate: {
      sites: "a file holding a non-null assertion, or an optional member read or call",
...

  {
    ...
    precision: "partial",
    applicabilityPredicate: {
      sites: "a file holding a function that returns an explicit null or undefined, from a return statement or an expression body",
      blind: "falling off the end of a function returns undefined with no site to count",
    },
    langs: ["js", "jsx"],
    run(program, add) {
      walk(program, (n, ctx) => {
...

  {
    ...
    applicabilityPredicate: {
      ...
      blind: "an indexed for loop is a third form the claim does not name and neither count reaches",
    },
    langs: ["js", "jsx"],
    run(program, add) {
      walk(program, (n, ctx) => {
...

  {
    ...
    precision: "partial",
    applicabilityPredicate: {
      sites: "a file exporting a top-level function or class, by name, as a default, or as a function-valued const",
      ...
      walk(program, (n) => {
        if (n.type === "ExportNamedDeclaration" && n.declaration) {
          ...
          if (d.type === "VariableDeclaration" &&
              d.declarations.some((x) => x.init && isFunctionLike(value(x.init)))) {
            return site(n, d.declarations[0]?.id?.name);
          }
        }
...
function attachedAbove(comments, start, source) {
  for (const c of comments) {
    ...
    if (!/^\s*$/.test(source.slice(c.end, start))) continue;
    const lineStart = source.lastIndexOf("\n", c.start) + 1;
    if (/^\s*$/.test(source.slice(lineStart, c.start))) return true;
  }
  return false;
}
