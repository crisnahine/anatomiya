import { walkRuby, constName, bodyOf } from "./ruby.mjs";
...
export const RUBY_DIMENSIONS = [
...

  {
    ...
    precision: "partial",
    applicabilityPredicate: {
      sites: "a Ruby file declaring a class with an entry method such as call or perform. Without that narrowing this would count every raise in the repository and measure nothing",
      blind: "a raise inside a helper the entry point calls is invisible from the entry method",
    },
    langs: ["ruby"],
    run(ast, add) {
      walkRuby(ast, (n, ctx) => {
        // `def self.call` is the commonest service entry form there is, and
        // refusing every receiver dropped it: the ratio was then stated over
        // whichever subset happens to write instance entry points, which is the
        // under-counted applicability this whole model exists to refuse. Only
        // `self`, because a def on any other receiver belongs to that object.
        const own = !n.receiver || n.receiver.t === "self";
        if (n.t !== "def" || !ctx.cls || !own || !ENTRY.test(n.name)) return;
        let raises = false;
        walkRuby(n.body, (m, mctx) => {
...

  {
    ...
    precision: "precise",
    applicabilityPredicate: {
      sites: "a Ruby file reading the clock through Time, Date or DateTime, including Time.zone.now",
      blind: null,
    },
    langs: ["ruby"],
    run(ast, add) {
