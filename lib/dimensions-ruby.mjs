...
export const RUBY_DIMENSIONS = [
...

  {
    ...
    precision: "precise",
    applicabilityPredicate: {
      sites: "a Ruby file declaring a class whose superclass is a constant, plain or scoped; a class naming no superclass is not a site",
      blind: null,
    },
    langs: ["ruby"],
    run(ast, add) {
...

  {
    ...
    precision: "precise",
    applicabilityPredicate: {
      sites: "a Ruby file calling include on a constant directly in a class or module body, once per constant; a call inside a method runs when the method does and is not a mixin the body declares",
      blind: null,
    },
    langs: ["ruby"],
    run(ast, add) {
      walkRuby(ast, (n, ctx) => {
        ...
        // One site per constant, because `include A, B` mixes in both and
        // counting the call once would drop half of what the body declared.
        for (const arg of (n.arguments && n.arguments.arguments) || []) {
          const mixin = constName(arg);
          if (mixin) add({ node: site(n), conforming: false, where: where(ctx), class: mixin });
        }
      });
    },
  },
