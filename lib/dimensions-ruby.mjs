...
export const RUBY_DIMENSIONS = [
...

  {
    ...
    // never conform above a half if each include is its own site (H14).
    groupedSites: true,
    claim: "classes here include <style>",
    counterClaim: null, // the other side is another module, which the learning already picks
    precision: "precise",
    applicabilityPredicate: {
      sites: "a Ruby class or module body's includes, counted once per body: every constant the body includes directly votes, and the body is one site. A call inside a method runs when the method does and is not a mixin the body declares",
      blind: null,
    },
    langs: ["ruby"],
    run(ast, add) {
      ...
      walkRuby(ast, (n, ctx) => {
        ...
        if (!ctx.cls || ctx.def) return;
        if (!groups.has(ctx.cls)) groups.set(ctx.cls, groups.size + 1);
        ...
        for (const arg of (n.arguments && n.arguments.arguments) || []) {
          ...
          if (mixin) {
            add({ node: site(n), conforming: false, where: where(ctx), class: mixin, group: groups.get(ctx.cls) });
          }
        }
      });
    },
