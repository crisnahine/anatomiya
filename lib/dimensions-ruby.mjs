...
export const RUBY_DIMENSIONS = [
  {
    ...
    precision: "precise",
    langs: ["ruby"],
    // Applicable: any file with a rescue. Candidates: the rescue clauses,
    // including each link of a multi-rescue chain.
    run(ast, add) {
      walkRuby(ast, (n, ctx) => {
...

  {
    ...
    precision: "partial", // a controller that wants the 404 is indistinguishable
    langs: ["ruby"],
    run(ast, add) {
      walkRuby(ast, (n, ctx) => {
...

  {
    ...
    precision: "partial", // an included concern can register callbacks from elsewhere
    langs: ["ruby"],
    run(ast, add) {
      walkRuby(ast, (n) => {
...

  {
    ...
    precision: "partial", // a raise inside a helper the entry point calls is invisible
    langs: ["ruby"],
    // Applicable: classes with an entry method. Without that predicate this
    // would count every raise in the repository and measure nothing.
    run(ast, add) {
      walkRuby(ast, (n, ctx) => {
        if (n.t !== "def" || !ctx.cls || n.receiver || !ENTRY.test(n.name)) return;
        let raises = false;
        walkRuby(n.body, (m, mctx) => {
