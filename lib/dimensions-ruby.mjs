...
import { CAPABILITY_WORDS, stemWords } from "./dimensions-capability.mjs";
...
export const RUBY_DIMENSIONS = [
...

  {
    ...
    precision: "partial",
    applicabilityPredicate: {
      sites: "a Ruby file calling puts, print, p, pp or warn with no receiver, or calling through a logger receiver",
...

  {
    ...
    precision: "partial",
    applicabilityPredicate: {
      sites: "a Ruby file calling Net::HTTP or URI.open, or making a verb-shaped call (get, post, put, patch, delete, head, request, call, perform, execute, fetch) through a constant or variable named client, http, api, request or fetcher",
...

  {
    ...
    precision: "precise",
    applicabilityPredicate: {
      sites: "a Ruby class declaration whose superclass is a constant, plain or scoped; a class naming no superclass is not a site",
      blind: null,
    },
    langs: ["ruby"],
    run(ast, add) {
      walkRuby(ast, (n) => {
        ...
        if (!base) return;
        add({ node: site(n), conforming: false, where: n.name, class: base });
      });
    },
  },
    ...
    run(ast, add) {
      ...
      walkRuby(ast, (n, ctx) => {
        if (n.t === "class" || n.t === "module") {
          ...
          if (!bodies.has(n)) {
            bodies.set(n, { node: n, ownedByClass: ctx.cls?.t === "class", group: bodies.size + 1, hits: [] });
          }
          return;
        }
        if (n.t !== "call" || n.name !== "include" || n.receiver) return;
      ...
      for (const body of bodies.values()) {
        ...
        // Three bodies have somewhere else to have got the mixin, and each was
        // a wrong finding on the corpus before it was excluded. A module
        // declaring nothing is namespacing, which is what modules are for:
        // counting `module Users` around a worker read forem's app/workers at
        // 88 of 176 and suppressed a claim true of every worker in it. A
        // subclass can be handed the mixin by its base, which is the other nine
        // of that same 97, and is the rule class_base states in the mirror. A
        // class inside a class is that class's helper rather than a peer of the
        // ones the claim is about. What is left is the class with nowhere else
        // to have got it, which is the forgotten include.
        if (body.node.t !== "class" || body.ownedByClass || constName(body.node.superclass)) continue;
        // No class key, so it votes for nothing in the learning and conforms to
        // no learned module in the fold. Both already read an absent class.
        add({ node: site(body.node), conforming: false, where: body.node.name ?? null, group: body.group });
      }
    },
  },
];
