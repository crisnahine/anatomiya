...
export const RUBY_DIMENSIONS = [
...

  {
    ...
    precision: "partial",
    applicabilityPredicate: {
      sites: "a Ruby file calling Net::HTTP or URI.open, or calling through a constant or variable whose name says client, http, api, request or fetcher",
      blind: "a client behind another name, and the wrapper's own Net::HTTP call inside its module, look the same from here",
    },
    langs: ["ruby"],
    run(ast, add) {
      walkRuby(ast, (n, ctx) => {
        ...
        if (clientReceiver(n.receiver, recv)) add({ node: site(n), conforming: true, where: where(ctx) });
      });
    },
  },
];
...
function clientReceiver(r, recv) {
  ...
  // call is the same vocabulary check the variable forms get.
