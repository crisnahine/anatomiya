...
/**
 * The modules a JSX area cannot be written without, so importing one says
...
export function commonImports(areaRecords) {
  ...
  for (const r of importing) {
    for (const module of new Set(r.facets.imports.filter((i) => !i.relative).map((i) => i.module))) {
      if (RUNTIME_MODULES.has(module)) continue;
 ...
 *
 * Counted in importing files rather than import statements, so one module that
 * pulls a name twice is one reader of it. This is the counted form of "check
 * before you create": five names with numbers beats a list of every export the
 * directory has.
 */
export function mostImported(areaRels, allRecords, corpusRels) {
  ...
  for (const file of areaRels) {
    for (const [name, who] of byFile.get(file) ?? []) {
      if (who.size >= MIN_IMPORTERS) rows.push({ name, file, importers: who.size });
    }
  }
  return rows
    .sort((a, b) => b.importers - a.importers || a.name.localeCompare(b.name) || a.file.localeCompare(b.file))
    .slice(0, 5);
}
