...
function readHead(path, bytes = HEAD_BYTES) {
  ...
  try {
    ...
    // file: Windows refuses to open a directory where POSIX opens it and fstat
    // says so, and a socket refuses everywhere, under an errno that differs
    // per platform. So the path is typed, and nothing is read after it: with
    // no read there is nothing for a swapped path to hand the wrong file to.
    try {
