...
export function classifyBasename(rel) {
  ...
  return stem === base ? null : classifyWord(stem);
}
...
export const NAMING_AST = [
  {
    ...
    run(program, add) {
      walk(program, (n, ctx) => {
        ...
        if (cls) add({ conforming: false, where: name, class: cls });
      });
    },
  },
  {
    ...
    run(program, add) {
      ...
      walk(program, (n) => {
        ...
        if (d?.type === "VariableDeclaration") {
          for (const decl of d.declarations) if (decl.id?.type === "Identifier") site(decl.id.name);
        } else if (d?.id?.name) {
          site(d.id.name);
        }
        // The exported name is the site: `export { foo as bar }` puts `bar` in
        // the module's surface, and `foo` stays this file's own business.
        for (const s of n.specifiers || []) site(s.exported?.name ?? s.exported?.value);
      });
    },
  },
];
