...
export function assertDeclaredFields(rows) {
  for (const d of rows) {
    ...
    // selected, and the oxc worker selects its own with no filter: a framework
    // row hosted there would count ungated in the scan while the check
    // filters, which is the split C8's row records.
    if (d.framework !== undefined && d.langs.some((l) => declOf(l).engine !== "prism")) {
