...
import { declOf, PARENT_SELECTING_ENGINES } from "./langs.mjs";
...
export function assertDeclaredFields(rows) {
  for (const d of rows) {
    ...
    // The framework filter is applied where an engine's dimensions are
    // selected, and a worker that selects its own gets no filter: a framework
    // row hosted there would count ungated in the scan while the check
    // filters, which is the split C8's row records.
    if (d.framework !== undefined && d.langs.some((l) => !PARENT_SELECTING_ENGINES.has(declOf(l).engine))) {
      throw new Error(`dimension ${d.key} declares a framework on an engine whose worker selects its own dimensions`);
    }
  }
  return rows;
}
