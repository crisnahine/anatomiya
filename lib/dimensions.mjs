...
assertPrecision([...ALL_DIMENSIONS, ...NAMING_CORPUS]);
assertApplicability([...ALL_DIMENSIONS, ...NAMING_CORPUS]);
assertTier([...ALL_DIMENSIONS, ...NAMING_CORPUS]);
assertClaimIsNotAVerdict([...ALL_DIMENSIONS, ...NAMING_CORPUS]);
assertLearnedRows([...ALL_DIMENSIONS, ...NAMING_CORPUS]);

/**
 * Which capabilities this repository has actually adopted: at least three
 * examined files already route through a wrapper. The filename vocabulary
 * (`capabilitiesIn`) is the candidate set; a repository holding a config.ts
 * nobody imports has a name, not a habit, and asking it the routing question
 * puts a line that can only read zero into its map (C14).
 */
export function adoptedCapabilities(records, candidates) {
  ...
  return new Set([...candidates].filter((c) => (adopters.get(c)?.size ?? 0) >= 3));
}
