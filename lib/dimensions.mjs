...
export const DIMENSIONS = [
  {
    ...
    precision: "precise",
    langs: ["js", "jsx"],
    // Applicable: any file with a catch clause. Candidates: the catch clauses.
    run(program, add) {
      walk(program, (n, ctx) => {
        if (!isCatch(n)) return;
...

  {
    ...
    counterClaim: null,
    precision: "partial", // a throw inside a helper the caller wraps is invisible
    langs: ["js", "jsx"],
    run(program, add) {
      walk(program, (n, ctx) => {
        if (!isThrow(n)) return;
...

  {
    ...
    counterClaim: null,
    precision: "partial", // a caller-level wrapper is invisible from here
    langs: ["js", "jsx"],
    run(program, add) {
      walk(program, (n, ctx) => {
...

  {
    ...
    counterClaim: null,
    precision: "partial", // destructured and interface-declared optionals are invisible
    langs: ["js", "jsx"],
    run(program, add) {
      walk(program, (n, ctx) => {
...
export function assertApplicability(rows) {
  for (const d of rows) {
    ...
    if (!a || typeof a.sites !== "string" || a.sites.trim().length <= 10) {
      throw new Error(`dimension ${d.key} declares no applicability.sites: say which files could participate`);
    }
    if (!("blind" in a)) {
      throw new Error(`dimension ${d.key} declares no applicability.blind: null, or what the predicate cannot see`);
    }
    ...
    if (d.precision === "partial" && (typeof a.blind !== "string" || a.blind.trim().length <= 10)) {
