...
export function readConfig(ts, root) {
  ...
  const result = ts.parseJsonConfigFileContent(parsed.config, host, root, undefined, configPath);
...
export function confinedCompilerHost(ts, root, options) {
  ...
  const allowed = (p) => insideRoot(root, p) || !relative(libDir, resolve(p)).startsWith("..");
