...
export const JSX_DIMENSIONS = [
  {
    ...
    precision: "precise",
    applicability: {
      sites: "a JSX file calling a React hook, either by its bare name or through the React namespace",
      blind: null,
    },
    langs: ["jsx"],
...

  {
    ...
    precision: "precise",
    applicability: {
      sites: "a JSX file passing an event handler prop whose value is a function, an identifier, or a non-computed member expression",
      blind: null,
    },
    langs: ["jsx"],
    run(program, add) {
...

  {
    ...
    precision: "partial",
    applicability: {
      sites: "a JSX file that already reaches a translation layer, so a repository without one produces no sites rather than a directory of zeros",
      blind: "a string handed to a component through a prop is invisible from the element that renders it",
    },
    langs: ["jsx"],
    /**
     * Applicable only inside files that already reach a translation layer. A
     * repository without one produces no sites rather than a directory of
     * zeros: one measured repository yields 311 candidates over 45 areas
     * without this gate, all non-conforming, and disappears with it.
     *
...

  {
    ...
    precision: "partial",
    applicability: {
      sites: "a JSX file binding a handler and passing it to a child component, counting only names this file bound",
      blind: "a handler defined in another file, or bound by destructuring, was decided where the value was created",
    },
    langs: ["jsx"],
    /**
     * Only names this file bound. An imported or destructured handler was
     * decided where the value was created, and counting it makes the number
     * grow with how many handlers a component receives rather than creates.
     */
    run(program, add) {
