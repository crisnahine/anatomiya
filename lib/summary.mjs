...
// Named because it is a write outside `.claude/rules/`, which is the one place
// A1 otherwise confines this tool to.
const HOOK_INSTALLED =
  ".claude/settings.local.json now re-delivers the map after every turn and every tool call";
...
export function scanSummary(result, plan, { dryRun = false, hook = null } = {}) {
  ...
  return {
    ...
    hookInstalled: hook?.changed === true,
...
export function scanLines(s) {
  ...
  // Only when it changed. A settings file this tool edited is a thing somebody
  // should be told about once, and saying it on every scan afterwards would
  // train a reader to skip the line that matters.
  if (s.hookInstalled) lines.push(HOOK_INSTALLED);
