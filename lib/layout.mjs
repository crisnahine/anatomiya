...
import { namesakeCompanions, namesakeIndex, stemOf } from "./companions.mjs";
import { baseOf, dirOf, withoutExtension } from "./paths.mjs";
 ...
 */
export function rootFacts(root, allFiles, testFiles = null, mirrored = null, byStem = null) {
  ...
  if (producers.length > 0 && everyTest.length > 0) {
    record.companions = namesakeCompanions(producers, everyTest, dir, byStem ?? namesakeIndex(everyTest));
  }
...
export function layoutFacts(
  ...
  const byStem = namesakeIndex(testFiles);
  return {
    size: files.length,
    minFiles,
    roots: roots.map((root) => rootFacts(root, files, testFiles, mirrored, byStem)),
    more,
