...
// forms spell, and the hyphen is how Ember spells it: discourse writes 4,000
// tests as `login-test.js` and no other signal in them says so.
const TEST_NAME = /[.-](test|spec|cy)\.|_(spec|test)\.rb$/;

/**
 * The directories a repository puts nothing but tests under, at the top level.
 *
 * Read for one thing only, the mirror below. A file being in one of these is
 * not what makes it a test: that was the rule, and it charged every factory,
 * fixture and page object to the runner.
 */
const TEST_ROOTS = new Set(["test", "tests", "spec"]);

const withoutExtension = (rel) => {
...
const helperFacet = (own, jsxFiles, mirrored) => {
  ...
  const modules = own.filter(
    (f) => MODULE_EXTS.has(extOf(f.rel)) && !f.facets?.jsx && !isTestFile(f, mirrored));
 ...
 */
export function rootFacts(root, allFiles, testFiles = null, mirrored = null) {
  const dir = root.dir ?? (root.path === "." ? "" : root.path);
  const own = root.files;
  const everyTest = testFiles ?? allFiles.filter((f) => isTestFile(f, mirrored));
  ...
  if (producers.length > 0 && everyTest.length > 0) {
    record.companions = namesakeCompanions(producers, everyTest, dir);
  }
 ...
 *
 * `mirrored` is taken rather than built where the caller has one: a scan needs
 * the same index for the area kinds, and the two of them walking the corpus
 * apart is one walk too many for an answer that cannot differ.
 */
export function layoutFacts(
  files,
  { minFiles = minRootFiles(files.length), budget = ROOT_BUDGET, mirrored = mirroredTests(files) } = {}
) {
  ...
  return {
    size: files.length,
    minFiles,
    roots: roots.map((root) => rootFacts(root, files, testFiles, mirrored)),
    more,
    tests: testsLine(files, mirrored),
  };
}
