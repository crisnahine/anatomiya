...
export async function scan(cwd, { onProgress = () => {}, baseRef = null, rubyGuards = null, deep = false } = {}) {
  ...
  // Which routing claims this repository can be asked at all: the filename
  // vocabulary names the candidates, and at least three files already routing
  // through a wrapper is what makes the habit real (C14).
  const capabilities = adoptedCapabilities(head.records, capabilitiesIn(files));
