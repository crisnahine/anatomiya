import { collect, gitRoot, countUntrackedSource } from "./corpus.mjs";
...
export async function scan(cwd, { onProgress = () => {}, baseRef = null, rubyGuards = null } = {}) {
  ...
  const head = await parseFiles(files, rubyGuards);
  ...
  const measured = await measureBaseline(root, state, areas, populations, head.parsed, rubyGuards);
  ...
  for (const area of areas) {
    ...
    const dims = reduceArea(area, areaParsed);
  ...
  return {
    ...
    corpus: { files: files.length, untracked, truncated, dropped, orphaned: areas.orphaned.length },
 ...
 */
async function parseFiles(files, rubyGuards = null) {
  ...
  const take = (r) => {
    ...
    // so a repository where nothing parsed reported a clean empty map.
    if (!r.ok && !r.crashed && !r.skipped) failed++;
  ...
  return { parsed, crashed, skipped, failed, missingParser, truncated };
}
 ...
 */
async function measureBaseline(root, state, areas, populations, headParsed, rubyGuards) {
  const out = new Map();
  ...
  try {
    if (stale.length) {
      ...
      for (const [rel, r] of (await parseFiles(blobs.files, rubyGuards)).parsed) parsed.set(rel, r);
    }
    ...
    for (const area of areas) {
      ...
      out.set(
        area.id,
        unread ? { gate: "population-change", dims: [] } : { gate: null, dims: reduceArea(area, usable) }
      );
    }
  } finally {
    blobs?.dispose();
...
/**
 * Languages this run read no file of, though the corpus holds some.
 *
 * Not a quality signal about the repository: five bad files out of six is a bad
 * repository, and six out of six with a missing interpreter is a blind run. The
 * difference matters because a blind run's areas all count nothing and would
 * otherwise be deleted as gone, which is `env -i PATH=/usr/bin:/bin` erasing a
 * correct Ruby map in the same run that reports it could not read one.
 */
function unreadableLangs(files, parsed) {
  ...
  for (const f of files) {
    ...
    if (r && r.ok) read.set(f.lang, (read.get(f.lang) || 0) + 1);
  }
  return [...total.keys()].filter((lang) => !read.get(lang)).sort();
}
