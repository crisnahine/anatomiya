...
import { resolveBaseline, baselinePopulation, baselineStates, measureBaseline, loadPin } from "./baseline.mjs";
...
export async function scan(cwd, { onProgress = () => {}, baseRef = null, rubyGuards = null } = {}) {
  ...
  // guard is.
  const measured = await measureBaseline(root, state, areas, populations, {
    headParsed: head.parsed,
    parse: (files) => parseFiles(files, { rubyGuards, frameworks }),
    reduce: (area, usable) => reduceArea(area, usable, { frameworks }),
  });
  ...
  for (const area of areas) {
    ...
    const gated = dims.map((d) => {
      ...
      return verdictFor(d, {
        baselineDim,
        scope,
        authors: authorCount((baselineDim || d).files, authors, baselineDim ? pinned.toCurrent : null),
        repoAuthors,
        historyRead,
        // A capped corpus answered for part of the repository, and a ratio over
        // an arbitrary subset rendered as a convention is worse than counts
        // (F7). Decided here rather than at render time, so the facts store and
        // the rendered map agree on what was stated.
        blocked: truncated ? "corpus-truncated" : suppression(state, population, baseline, baselineDim),
      });
    });
