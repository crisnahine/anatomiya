 ...
 */
export async function scan(cwd, { onProgress = () => {}, baseRef = null, guards = null, deep = false } = {}) {
  ...
  for (const area of areas) {
    ...
    // Ruby names its dependencies in a Gemfile and reaches them through
    // `require`, so an area holding no JavaScript is asked neither question:
    // an empty roster there would read as a measured "imports nothing".
    const hasImports = area.langs.some((l) => l === "js" || l === "jsx");
