...
import { fillClass, claimFor, CLASSES, NAMING_CORPUS } from "./dimensions-naming.mjs";
...
export async function check(cwd, { baseRef = null } = {}) {
  ...
  const pending = await pendingPaths(root);
  const examined = withPendingEdits(changed.filter((c) => wanted(c.path)), pending ?? []);
...
async function collect(root, { examined, areas, base, mode, added, fresh, caveats, frameworks, capabilities }) {
  ...
  for (const file of examined) {
    ...
    if (source === null) {
      caveats.push(file.tree ? `could not read ${file.path} in the working tree` : `could not read ${file.path} at HEAD`);
      continue;
    }
    ...
    // delete plus an add. E2 holds either way: only the head side is ever read
    // from the tree, so no edit of an agent's can move the base it is judged
    // against.
    if (mode === "compare" && file.from) {
      ...
      // A file only in the tree is the one case where an absent base is the
      // answer rather than a failure: it is new, so every site in it is.
      if (job.base === null && !file.tree) {
...
const CORPUS_ROWS = NAMING_CORPUS.filter((d) => d.kind === "corpus");
...
const constantsOf = (hits) => hits.map((h) => h.class ?? "").sort().join(" ");
...
async function pendingPaths(root) {
  ...
  if (!r.ok) return null;
  return parsePorcelainZ(r.out).filter(wanted);
}
...
function withPendingEdits(rows, pending) {
  ...
  for (const path of pending) {
    ...
    else byPath.set(path, { status: "M", path, from: path, tree: true });
  }
 ...
 *
 * The path comes from `git status`, so it is a repository-relative path git
 * itself printed, and it is joined rather than interpolated into a ref.
 */
