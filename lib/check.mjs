...
import { classifyBasename } from "./dimensions-naming.mjs";
import {
...
async function collect(root, { examined, areas, base, mode, added, fresh, caveats, frameworks, capabilities }) {
  ...
  for (const job of jobs) {
    ...
    if (nameDim && nameDim.learned !== undefined && !job.file.from) {
      const cls = classifyBasename(path);
      if (cls !== null && cls !== nameDim.learned) {
        ...
        findings.push({
          ...
          claim: `files here are named ${nameDim.learned}`,
...
function sidesFor(area) {
  ...
  for (const d of (area && area.dimensions) || []) {
    ...
    if (d.learned !== undefined) learned.set(d.key, d.learned);
  }
  ...
  for (const dim of dimensionsFor([lang], { frameworks, capabilities })) {
    ...
    try {
      dim.run(program, (hit) => {
        ...
        found.push({
          dimension: dim.key,
          claim: counter ? dim.counterClaim : dim.learnedClasses ? dim.claim.replace("<style>", cls) : dim.claim,
