...
async function collect(root, { examined, areas, base, mode, added, fresh, caveats, frameworks }) {
  ...
  const { results: parsed, missingParser } = await parseAll(entries);
  const findings = [];
...
async function parseAll(entries) {
  ...
  if (entries.length === 0) return { results: out, missingParser };
  ...
  try {
    ...
    // An absent parser is every file at once, and the per-file caveat reports it
    // as a repository whose files cannot be read. The scan already separates the
    // two; without this the check answers "no findings" to a run that parsed
    // nothing, which the command file tells the agent means the check ran.
  ...
  return { results: out, missingParser };
}
...
function unreadReason(parse) {
  if (!parse) return UNEXAMINED.failed;
  if (parse.crashed) return UNEXAMINED.crashed;
  if (parse.skipped) return UNEXAMINED.skipped;
  // Plural in the counted line, singular here, and the one word is the only
  // thing this surface changes.
  if (parse.errors) return UNEXAMINED.syntaxErrors.replace(/^hold /, "holds ");
  return UNEXAMINED.failed;
}
