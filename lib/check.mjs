...
export async function check(cwd, { baseRef = null, deep = false } = {}) {
  ...
  return {
    ...
    caveats,
    parse: { missingParser },
    semantic: { claims: semanticClaims, ran: deep },
    foreign,
    unknown,
  };
}
...
async function collect(root, { examined, areas, base, mode, added, fresh, caveats, frameworks }) {
  ...
  for (const job of jobs) {
    ...
    const head = violations(headParse.program, job.source, job.lang, keyPath, { sides, frameworks, stripped: headParse.stripped });
    ...
    if (mode === "added-lines") {
      ...
      introduced = newlyIntroduced(
        head,
        baseParse ? violations(baseParse.program, job.base, job.lang, keyPath, { sides, frameworks, stripped: baseParse.stripped }) : []
      );
    }
...
function violations(program, source, lang, keyPath, { sides = new Map(), frameworks, stripped = false } = {}) {
  const out = [];
...
export function formatReport(report) {
  ...
  if (report.semantic?.claims && !report.semantic.ran) {
    const n = report.semantic.claims;
    lines.push(
      `note: ${n} type-checked claim${n === 1 ? " was" : "s were"} not measured on this branch: rerun with --deep`
    );
  }
