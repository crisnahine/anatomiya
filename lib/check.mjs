...
import { gitRoot, isSource, isDenied, isExcludedDir, language } from "./corpus.mjs";
import { encode, encodePath } from "./encode.mjs";
import { PREFIX, statedSide } from "./render.mjs";
import { resolveBaseline, resolveBaseRef, mergeBase, BASE_REFS } from "./baseline.mjs";
...
export const FACTS = ".claude/anatomiya/facts.json";
...
export async function check(cwd, { baseRef = null } = {}) {
  ...
  if (!facts) caveats.push("no map on disk, so nothing was stated and nothing can be enforced");
  ...
  const changed = (from ? await changedFiles(root, from) : []).filter((c) => c.status !== "D");

  const examined = changed.filter((c) => wanted(c.path));

  await notePendingEdits(root, examined, caveats);

  const stale = await staleness(root, facts, base);
 ...
 * one hostile filename into two entries. A rename record is three NUL-separated
 * fields, everything else is two.
 */
function parseNameStatus(out) {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if (status === "R" || status === "C") {
      ...
      const to = parts[++i];
      if (!to) break;
      rows.push({ status, path: to, from });
    } else {
      const path = parts[++i];
      if (!path) break;
      rows.push({ status, path, from: status === "A" ? null : path });
    }
  }
  return rows;
}
...
async function collect(root, { examined, areas, base, mode, added, fresh, caveats }) {
  const jobs = [];
  for (const file of examined) {
    const lang = language(file.path);
    if (dimensionsFor([lang]).length === 0) continue; // nothing is claimed about this language
  ...
  for (const job of jobs) {
    entries.push({ key: `head:${job.file.path}`, lang: job.lang, source: job.head });
    if (job.base !== null) {
      entries.push({ key: `base:${job.file.path}`, lang: job.lang, source: job.base });
    }
  }

  const parsed = await parseAll(entries);
  const findings = [];
  ...
  for (const job of jobs) {
    ...
    if (!headParse || !headParse.ok || !headParse.program) {
      caveats.push(`${path} could not be parsed, so it was not checked`);
      continue;
    }
    ...
    const head = violations(headParse.program, job.head, job.lang, keyPath, sides);
    ...
    if (mode === "added-lines") {
      ...
      introduced = newlyIntroduced(
        head,
        baseParse ? violations(baseParse.program, job.base, job.lang, keyPath, sides) : []
      );
    }
...
function violations(program, source, lang, keyPath, sides = new Map()) {
  const out = [];
  for (const dim of dimensionsFor([lang])) {
...
// The extension the parser routes on. A blob is written under a generated
// name, so nothing else carries the language across.
...
/**
 * Parse every version, oxc in a pool of child processes and prism in one
 * streamed subprocess, the same split the scan uses.
 *
 * The blobs come from git rather than the working tree, so they are written to
 * a scratch directory first: the parser runs out of process because an oxc
 * segfault is not catchable, and it reads its input from a path.
 */
async function parseAll(entries) {
  const out = new Map();
  ...
  try {
    const files = entries.map((e, i) => {
      const abs = join(dir, `${i}.${scratchExt(e.lang)}`);
      writeFileSync(abs, e.source);
      return { rel: e.key, abs, lang: e.lang };
    });
    ...
    if (js.length) {
      // The tree, not just the counts: a finding here has to name the line it
      // sits on, and only the files a diff touched are parsed.
      const pool = createPool({ size: Math.min(4, js.length), withProgram: true });
      try {
        const results = await Promise.all(js.map((f) => pool.parse(f)));
        for (const r of results) out.set(r.rel, r);
      } finally {
        await pool.close();
      }
    }
    if (ruby.length) {
      for (const r of (await parseRuby(ruby)).results) out.set(r.rel, r);
    }
  } finally {
 ...
 */
async function staleness(root, facts, base) {
...
function readFacts(root) {
  try {
    const facts = JSON.parse(readFileSync(join(root, FACTS), "utf8"));
    return facts && Array.isArray(facts.areas) ? facts : null;
  } catch {
    return null;
  }
}
...
async function notePendingEdits(root, examined, caveats) {
  ...
  const dirty = r.out
    ...
    .filter((p) => examined.some((c) => c.path === p));
  if (dirty.length) {
    caveats.push(`${dirty.length} changed file(s) have uncommitted edits the check did not read`);
  }
}
...
async function git(root, args, maxBuffer = 32 * 1024 * 1024) {
  try {
    const { stdout } = await run("git", args, { cwd: root, maxBuffer, timeout: 30_000 });
    return { ok: true, out: stdout };
  } catch (err) {
    return { ok: false, out: String((err && err.stdout) || "") };
  }
}
