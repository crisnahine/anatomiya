...
import { classifyBasename, fillClass, CLASSES, NAMING_CORPUS } from "./dimensions-naming.mjs";
import {
...
export async function check(cwd, { baseRef = null } = {}) {
  ...
  // corpus shows a wrapper to route through (C8).
  const capabilities = new Set(await capabilitiesHere(root, facts, caveats));
...
async function capabilitiesHere(root, facts, caveats) {
  const stored = facts && facts.corpus && facts.corpus.capabilities;
  if (Array.isArray(stored)) return stored;
  try {
    return [...capabilitiesIn((await collectCorpus(root)).files)];
  } catch (err) {
    caveats.push(
      `the corpus could not be listed, so no routing claim was checked: ${err && err.message ? err.message : err}`
    );
    return [];
  }
}
...
async function collect(root, { examined, areas, base, mode, added, fresh, caveats, frameworks, capabilities }) {
  ...
  for (const job of jobs) {
    ...
    // The filename claim's site is the name itself, which no tree walk sees.
    // Only a file this branch created answers it: an old name predates the
    // branch, and a rename is the same file keeping its history.
    const nameDim = area && (area.dimensions || []).find((d) => d.key === "file_naming_case");
    if (nameDim && typeof nameDim.learned === "string" && CLASSES.includes(nameDim.learned)) {
      const cls = classifyBasename(path);
      const from = job.file.from;
      // A file this branch created answers the claim outright; a rename answers
      // it only when the name actually changed class, because the old name
      // predates the branch and a same-class rename kept the convention.
      const answers = !from || (from !== path && classifyBasename(from) !== cls);
      if (answers && cls !== null && cls !== nameDim.learned) {
        const verdict = severityFor({ path, oldPath: null }, { dim: nameDim, fresh });
        findings.push({
          severity: verdict.severity,
          reason: verdict.reason,
          path,
...
function sidesFor(area) {
  ...
  for (const d of (area && area.dimensions) || []) {
    ...
    // The value comes off a repository-committed record, so anything outside
    // the closed vocabulary is refused before it can reach a rendered claim
    // (F4): a value that is not a class enforces nothing.
    if (typeof d.learned === "string" && CLASSES.includes(d.learned)) learned.set(d.key, d.learned);
  }
...
function violations(program, source, lang, keyPath, { sides = new Map(), learned = new Map(), frameworks, capabilities, comments = [], stripped = false } = {}) {
  ...
  for (const dim of dimensionsFor([lang], { frameworks, capabilities })) {
    ...
    // A learned row with no learned class has no sentence to enforce: every
    // hit is a vote, and a vote is not a violation.
    const cls = learned.get(dim.key);
    if (dim.learnedClasses && cls === undefined) continue;
    ...
    try {
      dim.run(program, (hit) => {
        ...
        found.push({
          dimension: dim.key,
          claim: counter ? dim.counterClaim : dim.learnedClasses ? fillClass(dim.claim, cls) : dim.claim,
