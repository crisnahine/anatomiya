...
async function collect(root, { examined, areas, base, mode, added, fresh, caveats, frameworks, capabilities, pending }) {
  ...
  const atHead = await readAtRevision(root, head, [...headWanted.values()], REVISION_READ);
  const atBase = mode === "compare"
  ...
  const jobs = [];
  for (const { file, lang } of claimed) {
    const atSha = headBlobs.get(file.path);
    const source = file.tree ? await treeSource(root, file.path) : (atSha ? atSha.source : null);
    if (source === null) {
    ...
    const job = { file, lang, source, abs: atSha?.abs ?? null, base: null, baseAbs: null };
  ...
  const { records: parsed, missingEngines, missingParser, missingStripper } = await parseRead(entries, [atHead, atBase]);
  const findings = [];
...
/**
 * The parse, and the trees the blobs were read into, gone the moment it has
 * answered.
 *
 * The sources the report quotes are held in the parent, so nothing past this
 * needs the files: leaving them until the run ends would keep two copies of
 * every examined file in the temporary directory for the length of the check.
 */
async function parseRead(entries, reads) {
  try {
    return await parseAll(entries, { withProgram: true });
  } finally {
