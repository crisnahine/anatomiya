...
export async function check(cwd, { baseRef = null } = {}) {
  ...
  if (status === null) {
    caveats.push("the working tree's pending edits could not be listed, so only committed content was read");
  }
  ...
  if (unread) {
    caveats.push(`${unread} file(s) have uncommitted edits this run could not judge, because it found no base to compare them against`);
  }
...
async function treeSource(root, path) {
  ...
  try {
    // The same bound the committed side reads under. A file over it is refused
    // by the parse pool anyway, and the two sides disagreeing on what is too
    // big is what `limits.mjs` exists to stop.
