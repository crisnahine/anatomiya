...
import {
  gitBuffered, parseNameStatusZ, parsePorcelainZ, resolveBaseRef, mergeBase, filesAt, showBlob,
...
async function collect(root, { examined, areas, base, mode, added, fresh, caveats, frameworks }) {
  ...
  for (const job of jobs) {
    ...
    const head = violations(headParse.program, job.source, job.lang, keyPath, { sides, frameworks });
