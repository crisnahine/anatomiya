...
import { echoContext, installHook } from "./hook.mjs";
...
export async function runScan(cwd, { dryRun = false, deep = false } = {}) {
  ...
  // After the map and never before, since the hook re-delivers a file that has
  // to exist. It answers with a refusal rather than throwing one; the reasons
  // are in `installHook`.
