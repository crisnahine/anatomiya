...
import { commitHook, echoContext, planHook } from "./hook.mjs";
...
export async function runScan(cwd, { dryRun = false, deep = false } = {}) {
  ...
  // After the map, never before: the hook exists to re-deliver a file, and one
  // installed against a map that was never written would echo nothing on every
  // tool call for the life of the session. A blind run writes no map (A13), so
  // it installs no hook either.
  // Reported, never thrown. The map is the product and the hook is an addition
  // to it, so a settings file this refuses to touch must not take the whole
  // scan down with it: a run that wrote the entire map and then exited 1 over
  // an unrelated file is the map not arriving.
  const hook = installHook(result.root, { dryRun, blind: plan.blind });
  return { result, plan, hook, summary: scanSummary(result, plan, { dryRun, hook }) };
}

/**
 * Install the re-delivery hook, or say why not.
 *
 * A blind run wrote no map (A13) and a dry run wrote nothing at all, and a hook
 * pointed at a map that is not there echoes nothing on every tool call for the
 * life of the session.
 */
function installHook(root, { dryRun, blind }) {
  if (dryRun || blind) return { changed: false, refused: null };
  try {
    return { ...commitHook(root, planHook(root)), refused: null };
  } catch (err) {
    return { changed: false, refused: err.message };
  }
}
