...
export function reduceArea(area, parsed, { frameworks, tier = "syntactic" } = {}) {
  ...
  const dims = [...dimensionsFor(area.langs, { frameworks, tier }), ...pairingsFor(area.langs)];
  ...
  for (const dim of dims) {
    ...
    for (const file of parsed) {
      if (!wasRead(file) || !file.hits) continue;
...
export function verdictFor(
  ...
  const blocked = truncated
    ? "corpus-truncated"
    : (semanticBlock ?? (measured ? blockedFor(measured, baselineDim, dim) : null));
