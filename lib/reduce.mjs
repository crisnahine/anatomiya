...
/**
 * Which body a hit belongs to, keyed against a map already holding `n` keys.
 *
 * A hit carrying no group is its own site, which is what a row that does not
 * group already is. The two key shapes cannot collide, and both callers pass
 * the map's own size, so `#n` is free when it is asked for and the insert that
 * follows claims it.
 */
