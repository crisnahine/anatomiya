...
export function reduceArea(area, parsed, { frameworks } = {}) {
  ...
  for (const dim of dims) {
    ...
    let langFileCount = 0;
    for (const lang of langByRel.values()) if (dim.langs.includes(lang)) langFileCount++;
 ...
 *
 * The three were assembled in three modules, and only a full repository could
 * reach the assembly. The ordering is the point: a population nobody accepted
 * closes the slot before any gate is consulted, and it closes both sides.
 */
export function verdictFor(
  dim,
  { baselineDim = null, blocked = null, scope, authors, repoAuthors, historyRead = true } = {}
) {
  if (!scope) throw new TypeError("verdictFor needs the scope its gates read: { fileCount, dirCount }");
...
function distinctDirs(files = []) {
  ...
  for (const f of files) {
    ...
    const rel = typeof f === "string" ? f : f && f.rel;
    if (typeof rel !== "string") continue;
    const i = rel.lastIndexOf("/");
    set.add(i < 0 ? "." : rel.slice(0, i));
  }
  return set.size;
}
