...
import { NAMING_CORPUS, claimFor } from "./dimensions-naming.mjs";
...
/**
 * Which body a hit belongs to, keyed for a map holding `n` bodies already.
 *
 * A hit carrying no group is its own site, which is what a row that does not
 * group already is. The two key shapes cannot collide, and `#n` is claimed the
 * moment it is generated, so it is never generated twice.
 */
 ...
 * closed on the directories it was written for (H14).
 */
