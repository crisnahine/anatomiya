...
import { NAMING_CORPUS, fillClass } from "./dimensions-naming.mjs";
...
export function reduceArea(area, parsed, { frameworks, tier = "syntactic", capabilities } = {}) {
  ...
  for (const dim of dims) {
    ...
    out.push({
      ...
      ...(learned === undefined ? {} : { learned, claim: fillClass(dim.claim, learned) }),
    });
  }
...
export function verdictFor(
  ...
  const matchesDefault =
    states !== null &&
    (dim.learnedClasses
      ? dim.learned !== undefined && dim.learned === defaultClass(dim.key)
