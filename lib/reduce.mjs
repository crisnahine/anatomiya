...
import { language } from "./corpus.mjs";
import { defaultSideFor } from "./model-defaults.mjs";
 ...
 */
export function reduceArea(area, parsed, { frameworks, tier = "syntactic" } = {}) {
  ...
  const dims = [
    ...dimensionsFor(area.langs, { frameworks, tier }),
    ...
    ...NAMING_CORPUS.filter((d) => d.langs.some((l) => area.langs.includes(l))),
  ];
  ...
  for (const dim of dims) {
    ...
    if (dim.learnedClasses) {
      ...
      for (const [rel, hits] of perFile) {
        perFile.set(rel, hits.map((h) => ({ ...h, conforming: h.class === learned })));
      }
    }
    ...
    out.push({
      ...
      ...(learned === undefined ? {} : { learned, claim: dim.claim.replace("<style>", learned) }),
    });
  }
 ...
 */
export function learnClass(perFile) {
  const votes = new Map();
...
export function verdictFor(
  ...
  return {
    ...dim,
    ...g,
    matchesDefault: states !== null && states === defaultSide(dim.key),
