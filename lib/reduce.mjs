...
export function reduceArea(area, parsed, { frameworks } = {}) {
  ...
  // against. Both kinds produce the same hit shape, so the fold reads them
  // alike; the two places that do ask which kind this is are the denominator,
  // where an obligation speaks for a file whether or not it parsed, and the
  // companion count, which only an obligation has.
