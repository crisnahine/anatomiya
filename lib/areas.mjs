...
// The inverse of what `glob` and `patternsFor` build: a tail is an extension
// brace or one of the bare names, either of them optionally recursive.
const GLOB_TAIL = new RegExp(
  `(?:^|/)((?:\\*\\*/)?(?:\\*\\.[^/]*|${BARE_NAMES.map((n) => n.replace(/\./g, "\\.")).join("|")}))$`
);
...
export function splitGlob(pattern) {
  ...
  const m = GLOB_TAIL.exec(body);
  if (!m) return { negated, dir: body, tail: null };
  return { negated, dir: body.slice(0, m.index), tail: m[1] };
}
