...
import { baseOf, dirOf } from "./paths.mjs";
...
export function namesakeCompanions(sourceFiles, testFiles, rootPath = "", byStem = namesakeIndex(testFiles)) {
  ...
  // Null where the votes name the repository root: "under ." is not a place,
  // and the renderer drops the clause rather than print one.
  const ranked = [...votes].sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]));
  return { with: answered, of: sourceFiles.length, root: ranked.length === 0 ? null : ranked[0][0] || null };
}
