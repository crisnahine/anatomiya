...
export function namesakeCompanions(sourceFiles, testFiles, rootPath = "", byStem = namesakeIndex(testFiles)) {
  ...
  for (const f of sourceFiles) {
    ...
    for (const t of byStem.get(stemOf(f.rel)) ?? []) {
      ...
      const covered = !whole && (t.covers?.has(f.rel) || t.covers?.has(noExtension)) === true;
      ...
      // Nothing structural was shared, so the only place worth naming is the
      // one the test is actually kept in.
      if (prefix === null && covered) prefix = t.dir;
