...
export function namesakeCompanions(sourceFiles, testFiles, rootPath = "", byStem = namesakeIndex(testFiles)) {
  ...
  for (const f of sourceFiles) {
    ...
    for (const t of byStem.get(stemOf(f.rel)) ?? []) {
      ...
      // away a vote the next candidate was going to cast. The repository root
      // is not a place either, which `wholeRoot` spells as the empty string and
      // the renderer refuses to print, so it does not stop the walk and does not
      // vote: counting it only let it outrank a directory that is one.
