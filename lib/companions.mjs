...
const mirrors = (dir, bare) => {
  const flat = withoutTree(dir);
  return flat === bare || flat.endsWith(`/${bare}`);
};
...
// The test files by the stem each one answers, built once over the whole
// corpus because a scan asks the question per root and the map never differs.
...
export function namesakeCompanions(sourceFiles, testFiles, rootPath = "", byStem = namesakeIndex(testFiles)) {
  ...
  for (const f of sourceFiles) {
    ...
    for (const rel of byStem.get(stemOf(f.rel)) ?? []) {
      const dir = dirOf(rel);
      const whole = tail === "" || dir === tail || dir.endsWith(`/${tail}`);
      if (!whole && (bare === "" || !mirrors(dir, bare))) continue;
      matched = true;
      const prefix = whole ? wholeRoot(dir, tail) : mirrorRoot(dirOf(f.rel), dir);
  ...
  const [top] = [...votes].sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]));
