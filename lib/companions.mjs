...
export function namesakeCompanions(sourceFiles, testFiles, rootPath = "", byStem = namesakeIndex(testFiles)) {
  ...
  for (const f of sourceFiles) {
    ...
    // pairs 8 of its 15 that way. Top level on both sides is what keeps it
    // from reaching a package, where `apps/www` answered by a repository-wide
    // `test/` is the shape a monorepo keeps beside the package instead.
    ...
    for (const t of byStem.get(stemOf(f.rel)) ?? []) {
      const whole =
        tail === ""
          ? mirrors(t.bare, rootBare) || (flatPair && t.bare === "")
