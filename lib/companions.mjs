 ...
 * first of them and the corpus order is the filesystem's.
 */
export function namesakeIndex(testFiles) {
  ...
  for (const candidates of byStem.values()) candidates.sort((a, b) => a.rel.localeCompare(b.rel));
  return byStem;
}
...
export function namesakeCompanions(sourceFiles, testFiles, rootPath = "", byStem = namesakeIndex(testFiles)) {
  ...
  for (const f of sourceFiles) {
    ...
    for (const t of byStem.get(stemOf(f.rel)) ?? []) {
      ...
      const prefix = whole ? wholeRoot(t.dir, tail) : mirrorRoot(dirOf(f.rel), t.dir);
