...
const SOURCE = /\.(ts|tsx|js|jsx|mjs|cjs|rb|rake)$/;
...
export function language(path) {
  if (/\.(rb|rake)$/.test(path)) return "ruby";
...
export async function collect(root) {
  ...
  await lsFiles(root, (rel) => {
    if (isDenied(rel)) { dropped.denied = true; return true; }
    if (isExcludedDir(rel)) { dropped.excluded++; return true; }
    if (!isSource(rel)) { dropped.notSource++; return true; }
    const abs = safeResolve(root, rel);
    if (!abs) { dropped.escaped++; return true; }
