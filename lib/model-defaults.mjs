...
export function assertModelDefaults(table, registryKeys) {
  for (const [key, e] of table) {
    ...
    // stays `none` and the class rides beside it.
    if ("class" in e && (typeof e.class !== "string" || !e.class)) {
      throw new Error(`model-defaults ${key} declares class ${JSON.stringify(e.class)}, which is not a class name`);
    }
