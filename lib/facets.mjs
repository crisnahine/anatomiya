...
export function rubyFacets(program, rel = "") {
  ...
  walkRuby(program, (n, ctx) => {
    // Inside a `def` the call runs when that method does, so it declares no
    // case: a page object names its steps `context "..." do`. A class or module
    // body is where RSpec's own describes sit and stays a site. The same
    // altitude the JS half reads off `ctx.enclosing`.
