...
export function jsFacets({ program, module: mod }) {
  ...
  let jsx = false;
  // Whether this file carries type syntax of its own. The extension is a proxy
  // and it is wrong on Flow: a `.js` file annotated `// @flow` declares return
  // types the same way a `.ts` file does, and a row whose question is the
  // annotation has to ask it there too. Asked of the tree, which knows.
  let typed = false;
  let testCalls = false;
  ...
  return { jsx, typed, imports, exports, testRunner, testCalls, inlineHelpers };
}
...
// `test "x" do` is how Rails and minitest declare a case without a `def`, and
// `test_name "x" do` is Beaker's own spelling of the same shape for a Puppet
// acceptance scenario.
const RUBY_TEST_CALLS = new Set([...RSPEC_CALLS, "test", "test_name"]);

/**
 * The base classes a minitest file inherits, matched on the tail so a
 * `Rails::` or an application prefix still lands. Beside `Minitest::Test` and
 * `ActiveSupport::TestCase`, an integration, controller, job, mailer, view and
 * system test each name their own, and so do ActionCable's three and
 * ActionMailbox's.
 *
 * The list is what a repository has been seen to inherit, not a count of what
 * Rails ships. It said six and called that Rails's own number, and whitehall
 * inherits a seventh in 16 declarations. A missing entry reads a minitest file
 * as RSpec, so add to it on evidence rather than defending a total.
 */
...
// The name minitest gives its own classes, for the file that inherits from a
// base this list does not carry and sits outside any test directory.
const namedAsTest = (node) => /Test$/.test(constName(node?.name) ?? node?.name ?? "");
 ...
 * word. The directory half only trusts the tree's own top: Homebrew keeps its
 * whole app under `Library/Homebrew`, and a `def test_each` three directories
 * under ITS `test/` was an RSpec helper, not a minitest one.
 */
...
export function rubyFacets(program, rel = "") {
  ...
  walkRuby(program, (n, ctx) => {
    ...
    if (n.t === "call" && n.block && n.arguments && ctx.def === null && RUBY_TEST_CALLS.has(n.name)) {
      // The block is what separates the DSL from a method sharing its name (an
      // Interactor reads `context.amount`), and the argument is what separates
      // it from a bare block (a FactoryBot attribute reads `context { "tags" }`,
      // no description). A receiver other than `RSpec` is somebody else's
      // method for the same reason.
      const bare = !n.receiver || constName(n.receiver) === "RSpec";
      if (bare && n.name === "test") {
        // `ActiveSupport::Testing::Declarative` only runs mixed into a class
        // or module; a bare top-level `test` call is not Rails's macro. A class
        // body alone is not enough either: `test` is an ordinary word, and an
        // in-house rules engine naming its conditions with the same shape read
        // three authorization classes as minitest specs. So the macro asks for
        // the evidence `def test_*` already asks for, plus the name minitest
        // gives its own classes.
        if (ctx.cls && (isMinitestClass(ctx.cls) || minitestByPath(rel) || namedAsTest(ctx.cls))) {
          minitest = true;
          testCalls = true;
        }
      } else if (bare && n.name === "test_name") {
        beaker = true;
        testCalls = true;
      } else if (bare) {
        testCalls = true;
  ...
  return { testRunner: minitest ? "minitest" : rspec ? "rspec" : beaker ? "beaker" : null, testCalls };
}
