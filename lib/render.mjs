...
import { splitGlob } from "./areas.mjs";
 ...
 *
 * Only the directory half is encoded. The tail is a fixed pattern, and passing
 * it through would cost the leading `*`, which the encoder strips as a markdown
 * bullet: the result reads like a working glob and matches nothing. Where the
 * split itself lives, and why it is not a regex here, is in `areas.mjs`, which
 * is what builds the pattern.
 *
 * A leading `!` is the matcher's negation marker rather than part of any
 * directory name, so it is put back after encoding.
 */
function encodeGlob(g) {
  const { negated, dir, tail } = splitGlob(g);
  if (tail === null) return encodePath(String(g ?? ""));
  const head = dir ? `${encodePath(dir).slice(1, -1)}/` : "";
  return `"${negated ? "!" : ""}${head}${tail}"`;
}
