 ...
 */
function kindsLine(k) {
  if (k.exts.length === 0) return null;
  const parts = [extText(k), plural(k.tests.reduce((n, t) => n + t.files, 0), "test file")];
  if (k.companions) parts.push(`${k.companions.with} of ${k.companions.of} have a namesake test`);
...
export function renderArea(area) {
  ...
  // Counts rather than directives, so they outlive a suppressed count and give
  // way to a stated one: a directive is what the file exists to deliver, and a
  // roster is what makes the next file fit beside the ones already here.
  ...
  const body = fit(blocks, Math.max(MIN_BODY_LINES, MAX_LINES - head.length), (dropped) =>
    ...
    `and ${dropped} more not shown here${droppedKind(blocks.length, stated, dropped)}`
  );
  return [...head, ...body].join("\n").replace(/\n{3,}/g, "\n\n").trimEnd() + "\n";
}

/**
 * Whether the lines that did not fit were suppressed counts, stated directives,
 * or both. Counts sit last, so they go first.
 */
function droppedKind(total, stated, dropped) {
  const directives = Math.max(0, dropped - (total - stated));
  if (directives === 0) return ", all of them counts";
  if (directives === dropped) return ", all of them stated";
  return `, ${directives} of them stated`;
}
 ...
 */
const pathText = (p) => JSON.parse(encodePath(p));
...
function rootLine(r) {
  ...
  if (r.testRoot && r.tests.length > 0) {
    return `- ${pathText(r.path)}: ${specCount(r.files, r.tests[0].runner)}`;
  }
  ...
  if (r.companions) {
    ...
    parts.push(`${withTest} of ${of} have a namesake test${root ? ` under ${pathText(root)}` : ""}`);
  }
  if (r.helpers) {
    const { siblingModules, stems, inlineFiles } = r.helpers;
    parts.push(`${plural(siblingModules, ".ts module")} named ${stems.map((s) => encode(s)).join("/")}`);
 ...
 */
function testsLine(layout) {
  if (layout.tests.length === 0) return null;

  const shown = layout.tests.slice(0, TESTS_GROUPS);
  const parts = shown.map(
    (g, i) =>
      `${i === 0 ? specCount(g.files, g.runner) : runnerCount(g.files, g.runner)} under ${pathText(g.root)}`
  );
  ...
  if (top) {
    ...
    parts.push(`${top.companions.with} of ${plural(top.companions.of, noun)} have a namesake test`);
  }
  return `- tests: ${parts.join("; ")}`;
}

/**
 * Where this repository keeps its files, in at most fifteen lines.
 *
...
export function renderLayout(layout) {
  ...
  for (const r of layout.roots) lines.push(rootLine(r));

  const { roots: folded, files: leftover } = layout.more;
  if (folded > 0) {
    lines.push(
      `- and ${folded} more ${folded === 1 ? "directory" : "directories"} holding ${plural(leftover, "file")}`
    );
  } else if (leftover > 0) {
    ...
    lines.push(`- and ${leftover} more ${leftover === 1 ? "file" : "files"} in directories under the floor`);
  }

  const tests = testsLine(layout);
  ...
  const said = layout.principles.map((k) => sentence.get(k)).filter(Boolean);
...
export function renderOverview(result, files) {
  ...
  head.push(...renderLayout(result.layout));
...
/**
 * The other `.md` files in the rules directory, named here because this is the
 * file loading beside them.
 *
 * The two kinds keep their own sentences, because only one is true of each. A
 * file this tool did not write is somebody else's context to read; a file it did
 * write that no map lists is its own output from a scan whose record is gone,
 * and re-scanning is what clears it. Merged under the first sentence, the
 * always-loaded file asserted the tool had not written files it had.
 *
 * Only the first is named. The second is this tool's own output and the reader's
 * move is the same whichever file it is, so a count carries it; naming both cost
 * this section twice the lines on the file that pays for them every turn.
 */
/**
