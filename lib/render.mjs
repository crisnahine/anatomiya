...
/**
 * Files this map counted by extension in "## What lives where" but has no
 * language for at all, summed across every root that surfaced one.
 *
 * Undercounts by construction: a root the layout folded away, or where the
 * extension did not clear that root's own top two, holds none of its files
 * here. Still real for what it does count, which today is silence.
 */
export function unreadLanguageFiles(layout) {
...
function overviewTail(result, files) {
  ...
  const unread = unreadLanguageFiles(result.layout);
  if (unread.length) {
