...
export function unexaminedLines(parse) {
  const lines = [];
  if (parse.crashed) lines.push(`${parse.crashed} files ${UNEXAMINED.crashed}`);
  if (parse.failed) lines.push(`${parse.failed} files ${UNEXAMINED.failed}`);
  if (parse.syntaxErrors) lines.push(`${parse.syntaxErrors} files ${UNEXAMINED.syntaxErrors}`);
  if (parse.skipped) lines.push(`${parse.skipped} files ${UNEXAMINED.skipped}`);
  return lines;
}

// The phrase per cause, so the check can name one file with the sentence the
// summary and the overview use for a count of them.
export const UNEXAMINED = {
 ...
 */
const count = (xs, noun) => `${xs.length} ${noun}${xs.length === 1 ? "" : "s"}`;
