 ...
 *
 * The prefix earns its place for one job only: a single `.git/info/exclude`
 * line hides every generated file. It is not the ownership test, because a
 * hand-written file can take that name. Removal needs all three: the prefix,
 * the key, and being known to the scan.
 */
...
function encodeGlob(g) {
  const s = String(g ?? "");
  const tail = s.match(GLOB_TAIL);
  if (!tail) return encodePath(s);
  const dir = s.slice(0, tail.index);
  return `"${dir ? encodePath(dir).slice(1, -1) : ""}${tail[0]}"`;
}
  ...
  for (const [d, s] of counts) {
    lines.push(
      `${claimLine(s.claim)}: no convention. ` +
        `${s.conforming} of ${d.candidates} sites (${why(d, s.gate)})`
    );
  }
...
export function renderOverview(result, files) {
  ...
  // The overview is the one file loaded on every turn, so its listing is
  // bounded while the number of areas is not. Past the limit only the areas
  // that state something are named: an area carrying counts alone still has
  // its own path-scoped file, which loads when a file in it is read.
  const stated = (a) => a.dimensions.filter((d) => statedSide(d).states !== null).length;
  const listed = result.areas.length > OVERVIEW_AREAS
    ? result.areas.filter((a) => stated(a) > 0).slice(0, OVERVIEW_AREAS)
    : result.areas;
  ...
  lines.push(`- ${files.uncovered} source files sit in no area (too few per directory)`);
  lines.push("- memory, GC and I/O behaviour: runtime only, nothing static to count");
  if (result.parse.crashed) lines.push(`- ${result.parse.crashed} files could not be parsed`);
