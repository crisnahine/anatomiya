...
function fit(blocks, budget, more = null) {
  const kept = fitCount(blocks, budget, Boolean(more));
...
/**
 * How many of these blocks survive the budget: the drop rule, spelled once.
 *
 * Apart from `fit` because two readers need the same answer and only one of
 * them wants the lines. The check has to know which directives the file had no
 * room to state, and recomputing that from the same rule is the only shape
 * where the map and the check cannot disagree about what the agent was handed.
 *
 * `reserve` is the line `fit` gives back for its own notice, or it would be the
 * line that breaks the bound it exists to report.
 */
function fitCount(blocks, budget, reserve = false) {
  if (blocks.reduce((n, b) => n + b.length, 0) <= budget) return blocks.length;
  const room = reserve ? budget - 1 : budget;
  let used = 0;
...
function areaBlocks(area) {
  ...
  //
  // The kinds line takes the same floor a directive already gets: tried
  // unconditionally and left to `fit` below, so a `paths` list heavy enough to
  // eat the whole budget still leaves the one line a reader needs before
  // anything else this file says. Roster lines stay behind the room that
  // leaves: what a file imports is a second reading, not a first one.
  ...
  return { head, blocks, keys, stated, descriptions, kinds, budget: Math.max(MIN_BODY_LINES, MAX_LINES - head.length) };
}
...
export function droppedDirectives(area) {
  const { blocks, keys, budget } = areaBlocks(area);
  const kept = fitCount(blocks, budget, true);
  return new Set(keys.slice(kept).filter(Boolean));
}

export function renderArea(area) {
  const { head, blocks, keys, stated, descriptions, kinds, budget } = areaBlocks(area);
  ...
  const body = fit(blocks, budget, (dropped) =>
    ...
    `and ${dropped} more not shown here${droppedKind(blocks.length, keys, stated, descriptions, dropped)}`
  );
