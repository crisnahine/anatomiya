 ...
 * routed to the engine that hosts each language. `frameworks` filters the
 * framework-gated dimensions (C8) and is read only on the counts path.
 */
...
async function run(files, { withProgram, guards, frameworks }) {
  ...
  // Grouped by declared engine and run in registry order, which reproduces the
  // records insertion order downstream ties are derived from.
