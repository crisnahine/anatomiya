...
/**
 * The engines whose dimensions are selected in the parent, where a framework
 * filter can reach them. The oxc worker selects its own, unfiltered, so a
 * framework row may not live on a language it hosts; the registry refuses one
 * against this set rather than against a name spelled at the refusal site.
 */
export const PARENT_SELECTING_ENGINES = new Set(["prism"]);
