...
/**
 * Whether a rejected file is worth handing to the dialect stripper.
 *
 * Asked of the declaration rather than of a fixed table, so a language with no
 * dialect answers no without a list of its extensions existing anywhere else.
 */
export function mayRetryDialect(id, rel) {
  const decl = declOf(id);
  return decl.dialect !== null && decl.dialect.exts.includes(extIn(rel));
}
...
/** The path-only form of the dialect question, kept for callers with no language in hand. */
