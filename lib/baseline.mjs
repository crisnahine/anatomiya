import {
  changedSinceWorktree, diffRange, filesAt, isSha, resolveBaseRef, shaReachable, showBlob,
} from "./git.mjs";
import {
 ...
 *
 * `layout` is the corpus size the pin was taken over. The area floor is a step
 * function of it, so a caller deriving it from today's file count re-partitions
 * the repository on one added file, and every area then reads as a population
...
export async function resolve(root, { pin = loadPin(root), baseRef = null } = {}) {
  ...
  if (!reachable) {
    return state({ status: "unreachable", sha: pin.sha, countsOnly: true, areas: indexAreas(pin), layout: pin.corpus });
  }
...
function state(o) {
  return {
    ...
    // change against a pin that knew the old layout.
    layout: Number.isFinite(o.layout) ? o.layout : null,
    status: o.status,
    sha: o.sha ?? null,
  ...
  try {
    ...
    for (const area of areas) {
      ...
      for (const f of population.files) {
        ...
        // A file this tool cannot read at either revision is a blind spot it
        // has always had, not a population that moved. React is written in
        // Flow, which oxc does not take: 287 of its files never parse at the
        // pin or today, and reading that as a change closed 507 of its 986
        // slots. The current pass simply leaves such a file out of its counts,
        // and the two passes have to mean the same thing by it.
        const current = headParsed?.get(f.currentRel ?? f.rel) ?? null;
        if (current && current.ok !== true) continue;
        unread++;
      }

      // A baseline file that would not come back, or that this tool could read
      // at one revision and not the other, hides sites it can see on the other
      // side, and the sites it hides are the violating ones as often as the
