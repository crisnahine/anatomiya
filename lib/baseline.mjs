...
/**
 * Every git call reports its exit code beside its output, and no caller may
 * read stdout without looking at `ok`.
 *
 * `git merge-base` exits 1 with empty stdout and no stderr when the two commits
 * have no common ancestor. Code that captures stdout alone cannot tell that
 * from a successful answer, and passes "" downstream as if it were a sha.
 */
async function git(root, args, { encoding = "utf8", maxBuffer = 64 * 1024 * 1024 } = {}) {
  try {
    const { stdout } = await run("git", args, { cwd: root, encoding, maxBuffer });
...
export async function diffRange(root, from, to) {
  ...
  for (let i = 0; i < fields.length; ) {
    ...
    if (/^[RC]/.test(status)) {
      const from_ = fields[i + 1];
      const to_ = fields[i + 2];
      if (from_ && to_) {
        renames.set(to_, from_);
        changed.add(from_);
        changed.add(to_);
      }
      i += 3;
