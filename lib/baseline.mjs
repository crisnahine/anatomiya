...
/**
 * The pinned population's blobs on disk, where the parser pool reads them like
 * any other file.
 *
 ...
 * change. The name stays because this is what the baseline calls the step; the
 * reading of a revision it is one of two callers of lives in `revision.mjs`.
 */
export function materialize(root, sha, files, options) {
  return readAtRevision(root, sha, files, options);
}
