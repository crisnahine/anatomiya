...
export function formatDelta(delta) {
  ...
  // The count is the only fact those lines carried that the two lines above do
  // not. A re-pin keeps them: there the numbers differ from each other, and a
  // population change is what a human is being asked to accept.
  if (!delta.from) {
    lines.push(`${plural(delta.areas.length, "area")} ${delta.areas.length === 1 ? "enters" : "enter"} it`);
    return lines.join("\n");
  }
  lines.push("");
...
export async function resolve(root, { pin = loadPin(root), baseRef = null } = {}) {
  ...
  // read the branch's own commits as a map that had moved. Running from the
  // commit the two share fixes that and opens the mirror: after a squash merge
...
async function driftRange(root, pinSha, baseSha) {
  const shared = await mergeBase(root, pinSha, baseSha);
  const since = await diffRange(root, shared.found ? shared.sha : pinSha, baseSha);
  if (!since || !shared.found) return since;
  const differs = await diffRange(root, pinSha, baseSha);
  if (!differs) return since;
  const changed = new Set([...since.changed].filter((rel) => differs.changed.has(rel)));
