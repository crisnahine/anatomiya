 ...
 */
const MAY_HOLD_FLOW = /\.(js|jsx|mjs|cjs)$/;
 ...
 * would pay on every repository if this were a top-level import. A repository
 * with no Flow in it never loads this at all.
 *
...
process.on("message", async (job) => {
  ...
  try {
    ...
    let tree = result;
    if ((result.errors || []).length && MAY_HOLD_FLOW.test(rel)) {
      ...
      // types with whitespace rather than removing them, so every offset and
      // line number still lands where it does in the file on disk, which is
      // what lets the same walkers and the same reported nodes stand (B5).
      const strip = await ensureStripper();
      if (strip) {
        try {
          const stripped = strip(source, { all: true }).toString();
          const retried = parse(filename, stripped, parseOptions);
    ...
    const payload = {
      rel,
      ok: true,
      hits: collectHits(tree.program, dimensionsFor([lang])),
      errors,
