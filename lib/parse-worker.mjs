...
process.on("message", async (job) => {
  ...
  try {
    ...
    const filename = lang === "jsx" ? "f.tsx" : "f.ts";
    const result = parse(filename, source, parseOptions);

    const payload = {
      rel,
      ok: true,
      hits: runDimensions(result.program, lang),
...
/**
 * Every dimension that speaks this file's language, keyed for the reducer.
 *
 * A site keeps its conforming flag and the scope it sits in, and drops its
 * node: the reducer counts sites and names files, and never reads a position.
 */
function runDimensions(program, lang) {
  const hits = {};
  for (const dim of dimensionsFor([lang])) {
    ...
    try {
      ...
      // One dimension throwing on one odd tree costs that dimension's count for
      // this file, not the file and not the other twenty.
      continue;
    }
