...
export function createPool({ size, withProgram = false, execArgv = [] } = {}) {
  ...
  function spawn() {
    ...
    child.on("exit", (code, signal) => {
      ...
      if (!w.ready && ++stillborn >= MAX_STILLBORN) {
        return fail(`parser worker will not start${firstLine(w.stderr)}`);
      }
      spawn();
    });
  ...
  const target = Math.max(1, Math.min(64, Math.floor(size) || defaultSize()));
  for (let i = 0; i < target; i++) spawn();

  return { parse, close, size: target };
}

/** The worker's first real line of output, so a failure names its own cause. */
