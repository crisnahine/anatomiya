...
export async function parseRuby(
  ...
  await new Promise((resolve) => {
    ...
    const wall = setTimeout(
      () => stop("ruby ran past its wall clock"),
      guards.wallBaseMs + guards.wallPerFileMs * queued.length
    );
    wall.unref?.();
...
function take(out, onFile, seen, line, dimensions) {
  ...
  if (result.ok && program) {
    ...
    try {
      result.facets = rubyFacets(program);
    } catch {
...
// Re-exported rather than defined here: the facets read a tree the same way a
// dimension does, and this module reads the facets, so the walk cannot live on
// this side of that import.
export { walkRuby, bodyOf, constName, ownDef } from "./ruby-walk.mjs";
