 ...
 */
const RUBY_ENV = { PATH: process.env.PATH ?? "", LANG: "C" };
...
data.split("\\0").each_slice(2) do |rel, abs|
  ...
  begin
    ...
    emit({ "rel" => rel, "ok" => true, "ast" => conv(r.value),
           "errors" => r.errors.length, "length" => src.length })
...
export async function parseRuby(
  ...
  for (const f of queued) {
    ...
    deliver(out, onFile, { rel: f.rel, ok: false, error: out.error ?? "no result", crashed: true });
  }
