...
import { isTestFile, layoutFacts, minRootFiles, rootFacts } from "./layout.mjs";
...
export function roster({ files, others, records, truncated }) {
  ...
  const testFiles = corpus.filter((f) => isTestFile(f.rel, f.facets));

  return {
    layout: truncated ? notCounted(corpus) : counted(corpus),
    kinds: (area) =>
      rootFacts({ path: area.path, files: area.files.map((f) => byRel.get(f.rel)) }, corpus, testFiles),
  };
}
...
function counted(corpus) {
  const facts = layoutFacts(corpus);
  return { ...facts, principles: PRINCIPLES.filter((p) => p.when(facts)).map((p) => p.key), truncated: false };
}
