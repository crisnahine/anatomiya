...
// behind which kinds of file this repository holds and where. Both read back as
// null on an older record rather than as an empty roster, because an empty one
// is a repository with nothing in it and an absent one is a scan that never
// counted.
export const FACTS_SCHEMA = 11;
...
export function readFacts(root) {
  ...
  return { facts: withRoster(parsed), unreadable: null };
}

/**
 * The roster as a record older than schema 11 carries it, which is not at all.
 *
 * Filled in here rather than at each reader, so nothing downstream has to know
 * which version the field arrived in.
 */
function withRoster(parsed) {
  return {
    ...parsed,
    ...
    areas: parsed.areas.map((a) => (a ? { ...a, kinds: a.kinds ?? null } : a)),
  };
}
