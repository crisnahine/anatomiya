...
// such field and reads as absent rather than as zero. A version this reader has
...
export function schemaProblem(parsed) {
  if (!parsed || !Array.isArray(parsed.areas)) return "it is not a facts record";
  const schema = parsed.schema === undefined ? 1 : parsed.schema;
  if (!Number.isInteger(schema) || schema < 1 || schema > FACTS_SCHEMA) {
    return `it is schema ${JSON.stringify(parsed.schema)} and this build reads 1 to ${FACTS_SCHEMA}`;
  }
  return null;
}
...
export function readFacts(root) {
  ...
  if (problem === "it is not a facts record") return { facts: null, unreadable: null };
  if (problem) {
    return {
      facts: null,
      unreadable: `the map on disk cannot be read: ${problem}, so nothing was enforced from it: scan again with this build`,
    };
  }
