...
test("the widened fixture match stops at a real word boundary", () => {
  ...
  for (const p of [
    ...
    // "golden record" is master-data vocabulary, GoldenGate is an Oracle
    // product and the golden ratio is a number. Compounding on any word after
    // the hyphen reached all three; only the test compound was ever measured.
    "app/services/golden_record.rb",
    "lib/golden-gate/replicate.mjs",
    "lib/golden_ratio/compute.mjs",
  ]) {
    assert.equal(isExcludedDir(p), false, p);
  }
});
...
test("all three generated markers are read", async (t) => {
  ...
  // corpus with no trace anywhere in the map. It counts only beside a word
  // about generation, which is how every generator that stamps it writes it.
  const dir = repo(t, (d, { git, write }) => {
    write("a.ts", "// @generated\nexport const a = 1\n");
    write("b.ts", "// GENERATED FILE - DO NOT EDIT\nexport const b = 1\n");
...
test("a warning not to edit is not a claim that a generator wrote it", async (t) => {
  // A hand-written file whose head carries an operational warning, or one that
  // documents the markers this very filter looks for. Both vanished entirely,
  // not merely from the evidence: `collect` drops before any accounting, so
  // nothing in the map said they existed. This tool's own `lib/corpus.mjs`
  // holds the literal phrase and stayed in the corpus only because the comment
  // above it happens to run past the head cap.
  const dir = repo(t, (d, { git, write }) => {
    write("a.ts", "// DO NOT EDIT without updating ops/runbook.md\nexport const a = 1\n");
    // A module that documents the markers, the way this filter's own source
    // does, with the mention below the header where a generator never stamps.
    write("b.ts", `${"//\n".repeat(12)}const M = /@generated|DO NOT EDIT/\nexport const b = M\n`);
    git("add", "-A");
    git("commit", "-qm", "init");
  });
  ...
  assert.deepEqual(files.map((f) => f.rel), ["a.ts", "b.ts"]);
  assert.equal(dropped.generated, 0);
});
