...
const FIXTURE = [
  ...
  "  try {",
  "    for (const k of (Object.keys(c) as string[])) run(k)",
  "    const deep = (c as any).nested?.value",
  "    run(deep)",
  "  } catch (e) {",
  ...
  "}",
  "",
  "export function raise(bad: boolean) {",
  "  if (bad) throw new Error('bad')",
  "  throw 'a string'",
  "}",
  "",
  // The receiver names are how optional_chaining recognises an optional read.
  "export function reads(options: Config, params: Opts) {",
  "  const a = options?.name",
  "  const b = params.name",
  "  return [a, b]",
  "}",
  "",
  ...
  "  const v = await (run(n) as Promise<string>)",
