...
import { GATES, groupKey, isLearnedItself, wilsonLower } from "./reduce.mjs";
import {
...
async function collect(root, { examined, areas, base, mode, added, fresh, caveats, frameworks, capabilities, pending }) {
  ...
  try {
    ...
    for (const job of jobs) {
      ...
      const { sides, learned, kinds, qualified, stated } = sidesFor(area, ancestorsOf);
      const head = violations(headParse.program, job.source, job.lang, keyPath, { sides, learned, kinds, qualified, stated, frameworks, capabilities, comments: headParse.comments, stripped: headParse.stripped, rel: path, facets: headParse.facets });
      ...
      if (mode === "added-lines") {
        ...
        introduced = newlyIntroduced(
          head,
          // The head's facets for both revisions, the way `sides`, `learned`
          // and `kinds` are one map for both: the kind is a property of the
          // file under review, and answering it per revision skipped the whole
          // base side of a file that gained JSX on the branch, so every
          // pre-existing violation in it read as newly introduced.
          baseParse ? violations(baseParse.program, job.base, job.lang, keyPath, { sides, learned, kinds, qualified, stated, frameworks, capabilities, comments: baseParse.comments, stripped: baseParse.stripped, rel: path, facets: headParse.facets }) : []
        );
      }
...
function violations(program, source, lang, keyPath, { sides = new Map(), learned = new Map(), kinds = new Map(), qualified = new Map(), stated = new Set(), frameworks, capabilities, comments = [], stripped = false, rel = null, facets = null } = {}) {
  const out = [];
