...
/**
 * The spellings this repository actually gives the companions under one root.
 *
 * `companionRoot` learns where they are; this learns what they are called, on
 * the same argument and from the same evidence. The declared suffix is always
 * among them, as the prior the row is written with.
 *
 * A candidate is what a corpus file adds after a producer's whole tail, so
 * `app/models/address.rb` beside `spec/models/address_model_spec.rb` offers
...
export function companionSuffixes(corpus, { from, ext, companionSuffix }, root) {
  ...
  for (const rel of corpus) {
    ...
    tails.add(rel.slice(from.length + 1, -ext.length));
  }
  if (!tails.size) return [companionSuffix];

  // The corpus is walked once and each companion asked which producer it
  // extends, rather than each producer asked which companions extend it: the
  // second shape is a scan of the whole corpus per producer, and on a 20,000
  // file repository it cost 4.7 seconds a row against 0.9 before it existed.
  const counts = new Map();
  for (const c of corpus) {
    if (!c.startsWith(`${root}/`) || !c.endsWith(companionSuffix)) continue;
    const rest = c.slice(root.length + 1);
    const cut = rest.lastIndexOf("/");
    const dir = cut === -1 ? "" : rest.slice(0, cut + 1);
    const base = rest.slice(cut + 1);
    // The longest producer name this file extends, so a repository holding both
    // `address` and `address_model` reads the companion as the longer one's.
    for (let k = base.length - companionSuffix.length; k > 0; k--) {
      if (!tails.has(dir + base.slice(0, k))) continue;
      const suffix = base.slice(k);
      if (suffix !== companionSuffix && startsAtSeparator(suffix)) {
  ...
  const bar = Math.max(LEARNED_SUFFIX_FLOOR, LEARNED_SUFFIX_SHARE * tails.size);
  const learned = [...counts].filter(([, n]) => n >= bar).map(([suffix]) => suffix).sort();
  return [companionSuffix, ...learned];
}
...
function packagedPairings(corpus, pairing) {
  ...
  return [...prefixes].map((prefix) => {
    ...
    const root = companionRoot(corpus, pairing, prefix, named) ?? declared;
    return { from, root, suffixes: companionSuffixes(corpus, { ...pairing, from }, root) };
  });
}
...
export function pairingHits(corpus, pairing) {
  ...
  const hits = new Map();
  for (const { from, root, suffixes } of packagedPairings(corpus, pairing)) {
    for (const rel of corpus) {
      const companion = companionOf(rel, { ...pairing, from }, root);
      if (companion === null) continue;
      // Any spelling this root was seen to use answers the obligation; the
      // declared one is what a missing companion is named after, since it is
      // the spelling the row is written with.
 ...
 * than counted as failing, on the same argument the companion-suffix guard
 * below already makes: three unroutable files took a 62-of-62 claim to 62 of
 * 65, the evidence gate then read 0.8729 against the 0.90 bar, and the whole
 * claim went unstated over files nobody could ever satisfy.
 *
 * The noun comes off the row's own producer root, so `base_price_controller.rb`
 * stays a site and `app/models/base_controller.rb` does too: the name has to
 * match the directory it sits in.
 *
 * A structural test, "another class in the corpus names this class as its
 * superclass", selects the same files and cannot ship: `check` reports on files
 * the branch created, and no inheritance graph built at scan time holds a file
 * that does not exist yet.
 */
const isAbstractBase = (from, stem) => {
  const kind = from.slice(from.lastIndexOf("/") + 1).replace(/s$/, "");
  const base = stem.slice(stem.lastIndexOf("/") + 1);
  return base === "base" || base === `base_${kind}`;
};

/**
 * Every path that would satisfy this producer, one per spelling the root uses.
 *
...
export function companionOf(rel, { from, to, ext, companionSuffix }, root = to) {
  ...
  // Here rather than in `pairingHits`, because the check goes through
  // `pairingViolations` and a rule only the scan applies leaves the check
  // reporting a file the map never counted.
  if (isAbstractBase(from, stem)) return null;
