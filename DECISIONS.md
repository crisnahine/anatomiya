...
| C8 | A dimension whose claim belongs to a framework carries `framework`, and is offered only where the corpus shows that framework | Measured: `zone_aware_time` has no counter-claim, so off-Rails it can only ever print zero, and did: 123 sites on Homebrew, 197 on puppet, 97 on fastlane, 96 on chef, plus a NIT delivered onto a plain-Ruby branch. The signal is `app/models/`, `db/migrate/` or `config/application.rb` in the corpus, which separates 14 Rails from 5 plain across 19 repositories; read from the corpus, because rubocop's only `app/models` files are three fixtures the corpus drops. Only the two dimensions that cannot see their own context are marked | **done** `dimensions.mjs` `corpus.mjs` |
