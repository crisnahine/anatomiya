...
| C4 | Applicability floor: suppress the directive when `applicability / fileCount < 0.25` without a stated reason | v3 promised this floor and gave it no number, no gate, no storage, no test | **done** `reduce.mjs` (`GATES.minApplicabilityShare`, gate name `applicability`) |
...
| D2 | `candidates >= 6` | | **done** |
| D3 | **No single file supplies more than half of `candidates`** | v3's `files_conforming >= 3` does not block its own motivating case: 200 candidates in one file plus one each in 13 others gives 14 files, ratio 1.0, and every gate passes | **done** |
